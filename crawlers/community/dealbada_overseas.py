...
class DealbadaOverseasCrawler(BaseCrawler):
    ...
    HOTDEAL_URL = "https://www.dealbada.com/bbs/board.php?bo_table=deal_oversea"
    COMMUNITY_ID = 91
    ...
    def crawl(self, max_pages: int = 1, last_url: str = None) -> List[Dict]:
        ...
        Returns:
            크롤링된 딜 정보 리스트
        """
        logger.info(f"딜바다 해외 핫딜 크롤링 시작 (최대 {max_pages}페이지)")
        if last_url:
        ...
        logger.info(f"딜바다 해외 크롤링 완료: 총 {len(deals)}개 딜 수집")
        return deals
...
if __name__ == '__main__':
    ...
    crawler = DealbadaOverseasCrawler()
